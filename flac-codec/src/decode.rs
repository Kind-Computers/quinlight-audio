    ...
    bitstream_io::SignedInteger + Into<i64> + std::ops::AddAssign + std::ops::Neg<Output = Self>
{
    ...
    /// Wrapping addition required by the FLAC spec (RFC 9639 §10.2.5) for
    /// restoring original samples from residuals + LPC predictions at full
    /// bit-depth (e.g. 32-bit streams where i32 overflow is expected).
...
fn predict<I: SignedInteger>(coefficients: &[i64], qlp_shift: u32, channel: &mut [I]) {
    for split in coefficients.len()..channel.len() {
        ...
        residuals[0] = residuals[0].wrapping_add(I::from_i64(
            predicted
                .iter()
                .rev()
                .zip(coefficients)
