...
/*! \brief Render audio data (double precision)
 *
 * \param mod The module handle to work on.
 * \param samplerate Sample rate to render output. Should be in [8000,768000], but this is not enforced.
 * \param count Number of audio frames to render per channel.
 * \param interleaved_stereo Pointer to a buffer of at least count*2 elements that receives the interleaved stereo output in the order (L,R).
 * \return The number of frames actually rendered.
 * \retval 0 The end of song has been reached.
 * \remarks Double-precision samples are in the [-1.0..1.0] nominal range. They are not clipped to that range though and thus might overshoot.
 * \sa \ref libopenmpt_c_outputformat
 */
LIBOPENMPT_API size_t openmpt_module_read_interleaved_double_stereo( openmpt_module * mod, int32_t samplerate, size_t count, double * interleaved_stereo );
/*@}*/
 ...
 */
LIBOPENMPT_API double openmpt_module_get_current_channel_vu_mono( openmpt_module * mod, int32_t channel );
 ...
 * \remarks The returned value is solely based on the note velocity and does not take the actual waveform of the playing sample into account.
 */
LIBOPENMPT_API double openmpt_module_get_current_channel_vu_left( openmpt_module * mod, int32_t channel );
 ...
 * \remarks The returned value is solely based on the note velocity and does not take the actual waveform of the playing sample into account.
 */
LIBOPENMPT_API double openmpt_module_get_current_channel_vu_right( openmpt_module * mod, int32_t channel );
 ...
 * \remarks The returned value is solely based on the note velocity and does not take the actual waveform of the playing sample into account.
 */
LIBOPENMPT_API double openmpt_module_get_current_channel_vu_rear_left( openmpt_module * mod, int32_t channel );
 ...
 * \remarks The returned value is solely based on the note velocity and does not take the actual waveform of the playing sample into account.
 */
LIBOPENMPT_API double openmpt_module_get_current_channel_vu_rear_right( openmpt_module * mod, int32_t channel );
...
/*! \brief Get a sample's native sample rate
 *
 * \param mod The module handle to work on.
 * \param index The sample whose rate should be retrieved (0-based).
 * \return The sample rate in Hz, or 0 on failure.
 */
LIBOPENMPT_API int32_t openmpt_module_get_sample_rate( openmpt_module * mod, int32_t index );

/*! \brief Get a sample's length in frames
 *
 * \param mod The module handle to work on.
 * \param index The sample whose length should be retrieved (0-based).
 * \return The sample length in frames, or 0 on failure.
 */
LIBOPENMPT_API int64_t openmpt_module_get_sample_length_frames( openmpt_module * mod, int32_t index );

/*! \brief Get a sample's number of channels
 *
 * \param mod The module handle to work on.
 * \param index The sample whose channel count should be retrieved (0-based).
 * \return 1 for mono, 2 for stereo, or 0 on failure.
 */
LIBOPENMPT_API int32_t openmpt_module_get_sample_channels( openmpt_module * mod, int32_t index );

/*! \brief Resolve the sample mapped by an instrument for a specific note
 *
 * \param mod The module handle to work on.
 * \param instrument_index The 0-based instrument index.
 * \param note The libopenmpt/OpenMPT note value (1 = lowest playable note).
 * \return The 0-based sample index, or -1 if the instrument/note combination is unmapped.
 */
LIBOPENMPT_API int32_t openmpt_module_get_instrument_keyboard_sample( openmpt_module * mod, int32_t instrument_index, int32_t note );

/*! \brief Read sample data as normalized float
 *
 * Reads the raw sample data and converts it to float in the range [-1.0, 1.0].
 * For stereo samples, frames are interleaved (L, R, L, R, ...).
 * \param mod The module handle to work on.
 * \param index The sample to read (0-based).
 * \param buffer Pointer to a float buffer. Must hold at least length_frames * channels elements.
 * \param buffer_frames Maximum number of frames to read.
 * \return The number of frames actually read, or 0 on failure.
 */
LIBOPENMPT_API int64_t openmpt_module_read_sample_data( openmpt_module * mod, int32_t index, double * buffer, int64_t buffer_frames );

/*! \brief Replace sample data from float buffer
 *
 * Replaces the sample's waveform data. The float data is stored as float64 runtime sample data
 * and is quantized back to integer only when saving to tracker formats.
 * Loop points are scaled proportionally based on the sample rate change.
 * For stereo data, frames should be interleaved (L, R, L, R, ...).
 * \param mod The module handle to work on.
 * \param index The sample to replace (0-based).
 * \param data Pointer to float data in [-1.0, 1.0] range, interleaved if stereo.
 * \param length_frames Number of frames in the data.
 * \param channels 1 for mono, 2 for stereo.
 * \param new_sample_rate The new sample rate in Hz.
 * \return 1 on success, 0 on failure.
 */
LIBOPENMPT_API int openmpt_module_replace_sample_data( openmpt_module * mod, int32_t index, const double * data, int64_t length_frames, int32_t channels, int32_t new_sample_rate );

/*! \brief Replace a sample's waveform data without scaling loop points.
 *
 * Same as openmpt_module_replace_sample_data but skips automatic loop point
 * scaling.  The caller is expected to set loop points explicitly via
 * openmpt_module_set_sample_loop_points afterwards.
 * \param mod The module handle to work on.
 * \param index The sample to replace (0-based).
 * \param data Pointer to float data in [-1.0, 1.0] range, interleaved if stereo.
 * \param length_frames Number of frames in the data.
 ...
 * \return 1 on success, 0 on failure.
 */
LIBOPENMPT_API int openmpt_module_replace_sample_data_raw( openmpt_module * mod, int32_t index, const double * data, int64_t length_frames, int32_t channels, int32_t new_sample_rate );

/*! \brief Set explicit loop points on a sample.
 *
 * Sets the normal and sustain loop regions on a sample.
 * Loop mode values: 0 = no loop, 1 = forward, 2 = ping-pong.
 * \param mod The module handle to work on.
 * \param index The sample index (0-based).
 * \param loop_start Normal loop start in frames.
 * \param loop_end Normal loop end in frames.
 * \param loop_mode Normal loop mode (0/1/2).
 * \param sustain_start Sustain loop start in frames.
 * \param sustain_end Sustain loop end in frames.
 * \param sustain_mode Sustain loop mode (0/1/2).
 * \return 1 on success, 0 on failure.
 */
LIBOPENMPT_API int openmpt_module_set_sample_loop_points( openmpt_module * mod, int32_t index, int64_t loop_start, int64_t loop_end, int32_t loop_mode, int64_t sustain_start, int64_t sustain_end, int32_t sustain_mode );

/*! \brief Reset per-channel mixer state tied to the given sample's rate.
 *
 * Walks active channels currently playing the specified sample and clears state
 * that depends on the sample rate: IIR filter memory, β-shear resampler
 * acceleration history, output DC-offset buffers, and in-flight volume ramps.
 * Sample playback position, envelopes, and LFO phases are left untouched so the
 * note continues seamlessly. Intended to be called right after
 * openmpt_module_replace_sample_data / _raw when the new rate differs from the
 * old one, eliminating the filter / resampler transients that otherwise occur.
 * \param mod The module handle to work on.
 * \param index The sample index (0-based).
 * \return 1 on success, 0 on failure.
 */
LIBOPENMPT_API int openmpt_module_refresh_channels_for_sample( openmpt_module * mod, int32_t index );

/*! \brief Query whether linear slides are enabled on the loaded module.
 *
 * \param mod The module handle.
 * \return 1 if linear slides are enabled, 0 otherwise.
 */
LIBOPENMPT_API int openmpt_module_get_linear_slides( openmpt_module * mod );

/*! \brief Toggle linear slides on the loaded module.
 *
 * \param mod The module handle.
 * \param enabled Non-zero to enable linear slides, 0 to disable them.
 * \return 1 on success, 0 on failure.
 */
...
/*! \brief Query whether OpenMPT AGC is enabled for the loaded module.
 *
 * \param mod The module handle.
 * \return 1 if AGC is enabled, 0 otherwise.
 */
LIBOPENMPT_API int openmpt_module_get_agc_enabled( openmpt_module * mod );

/*! \brief Toggle OpenMPT AGC for the loaded module.
 *
 * \param mod The module handle.
 * \param enabled Non-zero to enable AGC, 0 to disable it.
 * \return 1 on success, 0 on failure.
 */
LIBOPENMPT_API int openmpt_module_set_agc_enabled( openmpt_module * mod, int enabled );

/*! \brief Query the active OpenMPT AGC profile.
 *
 * \param mod The module handle.
 * \return One of OPENMPT_MODULE_AGC_PROFILE_* values, or -1 on failure.
 */
LIBOPENMPT_API int openmpt_module_get_agc_profile( openmpt_module * mod );

/*! \brief Select the OpenMPT AGC profile.
 *
 * \param mod The module handle.
 * \param profile One of OPENMPT_MODULE_AGC_PROFILE_* values.
 * \return 1 on success, 0 on failure.
 */
LIBOPENMPT_API int openmpt_module_set_agc_profile( openmpt_module * mod, int profile );

/*! \brief Save the module (with any modified samples) to memory.
 *
 * \param mod The module handle.
 * \param buffer Output buffer, or NULL to query required size.
 * \param buffer_size Size of the output buffer in bytes.
 * \return Number of bytes written (or required if buffer is NULL), or -1 on failure.
 */
LIBOPENMPT_API int64_t openmpt_module_save_to_memory( openmpt_module * mod, void * buffer, int64_t buffer_size );

/*! \brief Save the loaded module in its exact source format, if writable.
 *
 * \param mod The module handle.
 * \param buffer Output buffer, or NULL to query required size.
 * \param buffer_size Size of the output buffer in bytes.
 * \return Number of bytes written (or required if buffer is NULL), or -1 if the source format is not directly writable.
 */
LIBOPENMPT_API int64_t openmpt_module_save_loaded_format_to_memory( openmpt_module * mod, void * buffer, int64_t buffer_size );

/*! \brief Save the module in libopenmpt's best directly writable fallback format.
 *
 * \param mod The module handle.
 * \param buffer Output buffer, or NULL to query required size.
 * \param buffer_size Size of the output buffer in bytes.
 * \return Number of bytes written (or required if buffer is NULL), or -1 on failure.
 */
LIBOPENMPT_API int64_t openmpt_module_save_best_format_to_memory( openmpt_module * mod, void * buffer, int64_t buffer_size );

/*! \brief Get the writable extension for the exact loaded format.
 *
 * Returns an empty string if the loaded format is not directly writable.
 */
LIBOPENMPT_API const char * openmpt_module_get_loaded_format_extension( openmpt_module * mod );

/*! \brief Get the writable extension for the best fallback save format.
 */
LIBOPENMPT_API const char * openmpt_module_get_best_save_format_extension( openmpt_module * mod );

/*! \brief Save a detached render snapshot of the current module state to memory.
 *
 * The snapshot is serialized using the closest directly saveable module format so
 * that modified samples and current playback flags can be rendered from a fresh
 * module instance without reusing the live playback object.
 *
 * \param mod The module handle.
 * \param buffer Output buffer, or NULL to query required size.
 * \param buffer_size Size of the output buffer in bytes.
...
#define OPENMPT_MODULE_SAMPLE_FORMAT_FLOAT64 3

/*! \brief Get the bit depth of a sample.
 *
 * \param mod The module handle.
 * \param index The sample index (0-based).
 * \return 8 or 16 (target save bit depth), or 0 on failure.
 */
LIBOPENMPT_API int32_t openmpt_module_get_sample_bits_per_sample( openmpt_module * mod, int32_t index );

/*! \brief Get the runtime sample format of a sample.
 *
 * \param mod The module handle.
 * \param index The sample index (0-based).
 * \return One of OPENMPT_MODULE_SAMPLE_FORMAT_* values, or -1 on failure.
 */
LIBOPENMPT_API int32_t openmpt_module_get_sample_format( openmpt_module * mod, int32_t index );

/*! \brief Check if a sample has a loop defined.
 *
 * \param mod The module handle.
 * \param index The sample index (0-based).
 * \return 1 if the sample has a loop, 0 otherwise.
 */
LIBOPENMPT_API int openmpt_module_has_sample_loop( openmpt_module * mod, int32_t index );

/*! \brief Get a sample's loop start in frames.
 *
 * \param mod The module handle.
 * \param index The sample index (0-based).
 * \return The loop start in frames, or 0 if the sample has no valid loop.
 */
LIBOPENMPT_API int64_t openmpt_module_get_sample_loop_start( openmpt_module * mod, int32_t index );

/*! \brief Get a sample's loop end in frames.
 *
 * \param mod The module handle.
 * \param index The sample index (0-based).
 * \return The loop end in frames, or 0 if the sample has no valid loop.
 */
LIBOPENMPT_API int64_t openmpt_module_get_sample_loop_end( openmpt_module * mod, int32_t index );

/*! \brief Get a sample's loop mode.
 *
 * \param mod The module handle.
 * \param index The sample index (0-based).
 * \return 0 for no loop, 1 for forward loop, 2 for ping-pong loop.
 */
LIBOPENMPT_API int32_t openmpt_module_get_sample_loop_mode( openmpt_module * mod, int32_t index );

/*! \brief Check if a sample has a sustain loop defined.
 *
 * \param mod The module handle.
 * \param index The sample index (0-based).
 * \return 1 if the sample has a sustain loop, 0 otherwise.
 */
LIBOPENMPT_API int openmpt_module_has_sample_sustain_loop( openmpt_module * mod, int32_t index );

/*! \brief Get a sample's sustain loop start in frames.
 *
 * \param mod The module handle.
 * \param index The sample index (0-based).
 * \return The sustain loop start in frames, or 0 if the sample has no valid sustain loop.
 */
LIBOPENMPT_API int64_t openmpt_module_get_sample_sustain_loop_start( openmpt_module * mod, int32_t index );

/*! \brief Get a sample's sustain loop end in frames.
 *
 * \param mod The module handle.
 * \param index The sample index (0-based).
 * \return The sustain loop end in frames, or 0 if the sample has no valid sustain loop.
 */
LIBOPENMPT_API int64_t openmpt_module_get_sample_sustain_loop_end( openmpt_module * mod, int32_t index );

/*! \brief Get a sample's sustain loop mode.
 *
 * \param mod The module handle.
 * \param index The sample index (0-based).
 * \return 0 for no sustain loop, 1 for forward loop, 2 for ping-pong loop.
 */
LIBOPENMPT_API int32_t openmpt_module_get_sample_sustain_loop_mode( openmpt_module * mod, int32_t index );

/*! \brief Set a command field in a pattern cell.
 *
 * \param mod The module handle.
 * \param pattern The pattern index.
 * \param row The row index.
 * \param channel The channel index.
 * \param command The command field (OPENMPT_MODULE_COMMAND_*).
 * \param value The value to set.
 * \return 1 on success, 0 on failure.
 */
LIBOPENMPT_API int openmpt_module_set_pattern_row_channel_command( openmpt_module * mod, int32_t pattern, int32_t row, int32_t channel, int command, uint8_t value );

LIBOPENMPT_API int32_t openmpt_module_get_current_channel_sample( openmpt_module * mod, int32_t channel );
