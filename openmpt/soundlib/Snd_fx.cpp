...
static PitchT ScaleXMSlideAmount(const CSoundFile &, ModChannel &, PitchT amount)
{
	return amount;
}
...
void CSoundFile::InstrumentChange(ModChannel &chn, uint32 instr, bool bPorta, bool bUpdVol, bool bResetEnv) const
{
	...
	if(newTuning)
	{
		chn.nC5Speed = pSmp->GetPlaybackC5Speed(GetType());
		chn.m_CalculateFreq = true;
		chn.nFineTune = 0;
	} else if(!bPorta || sampleChanged || !(GetType() & (MOD_TYPE_MOD | MOD_TYPE_XM)))
	{
		const auto [playbackTranspose, playbackFineTune] = pSmp->GetPlaybackTransposeFineTune(GetType());
	...
	chn.nTranspose = UseFinetuneAndTranspose() ? pSmp->GetPlaybackTransposeFineTune(GetType()).first : 0;
...
void CSoundFile::NoteChange(ModChannel &chn, int note, bool bPorta, bool bResetEnv, bool bManual, CHANNELINDEX channelHint) const
{
	...
	if(!bPorta && (GetType() & (MOD_TYPE_MOD | MOD_TYPE_XM | MOD_TYPE_MED | MOD_TYPE_MT2)))
	{
		if(pSmp)
		{
			const auto [playbackTranspose, playbackFineTune] = pSmp->GetPlaybackTransposeFineTune(GetType());
	...
	PitchT period = GetPeriodFromNote(note, chn.nFineTune, chn.nC5Speed);
	...
	if(m_playBehaviour[kITPitchPanSeparation] && pIns && pIns->nPPS)
	{
		...
		{
			PanT tempPan = chn.nPan;
			ProcessPitchPanSeparation(tempPan, origNote, *pIns);
			chn.nPan = static_cast<int32>(tempPan);
		}
	}
	...
	{
		chn.triggerNote = true;
		chn.nLeftVU = chn.nRightVU = 2.0;  // Sentinel > 1.0: snap VU to current level on next tick
...
CHANNELINDEX CSoundFile::GetNNAChannel(CHANNELINDEX nChn) const
{
	...
	if(nChn < m_PlayState.Chn.size())
	{
		...
		vol = (static_cast<int32>(srcChn.nRealVolume) << 9) | srcChn.nVolume;
	}
	...
	for(CHANNELINDEX i = GetNumChannels(); i < m_PlayState.Chn.size(); i++)
	{
		...
		int32 v = (static_cast<int32>(c.nRealVolume) << 9) | c.nVolume;
...
CHANNELINDEX CSoundFile::CheckNNA(CHANNELINDEX nChn, uint32 instr, int note, bool forceCut)
{
	...
	{
		if(!srcChn.nLength || srcChn.dwFlags[CHN_MUTE] || (srcChn.rightVol == 0 && srcChn.leftVol == 0))
			return CHANNELINDEX_INVALID;
		...
		if(triggerNote)
		{
			...
			if(ModCommand::IsNote(note) && m_playBehaviour[kFT2Transpose])
			{
				...
				if(instr && !bPorta)
				{
					...
					if(sample > 0)
						transpose = GetSample(sample).GetPlaybackTransposeFineTune(GetType()).first;
				}
...
void CSoundFile::PortamentoUp(PlayState &playState, CHANNELINDEX nChn, ModCommand::PARAM param, const bool doFinePortamentoAsRegular) const
{
	...
	{
		DoFreqSlide(chn, chn.nPeriod, ScaleXMSlideAmount(*this, chn, param * 4));
	}
}
...
void CSoundFile::ExtraFinePortamentoUp(ModChannel &chn, ModCommand::PARAM param) const
{
	...
	if(chn.isFirstTick && chn.nPeriod && param)
		DoFreqSlide(chn, chn.nPeriod, ScaleXMSlideAmount(*this, chn, param));
}
...
void CSoundFile::ExtraFinePortamentoDown(ModChannel &chn, ModCommand::PARAM param) const
{
	...
	if(chn.isFirstTick && chn.nPeriod && param)
	{
		DoFreqSlide(chn, chn.nPeriod, ScaleXMSlideAmount(*this, chn, -static_cast<int32>(param)));
		if(chn.nPeriod > 0xFFFF && !m_playBehaviour[kPeriodsAreHertz] && (!m_SongFlags[SONG_LINEARSLIDES] || GetType() == MOD_TYPE_XM))
			chn.nPeriod = 0xFFFF;
	}
}
...
int32 CSoundFile::TonePortamento(PlayState &playState, CHANNELINDEX nChn, uint16 param) const
{
	...
	{
		const PitchT actualDelta = ScaleXMSlideAmount(*this, chn, PeriodsAreFrequencies() ? delta : -delta);
...
void CSoundFile::TonePortamentoWithDuration(ModChannel &chn, uint16 param) const
{
	...
	} else if(chn.nPeriod && chn.nPortamentoDest)
	{
		// Run portamento
		chn.dwFlags.set(CHN_PORTAMENTO);
		const PitchT actualDelta = ScaleXMSlideAmount(*this, chn, PeriodsAreFrequencies() ? chn.portamentoSlide : -chn.portamentoSlide);
		if(chn.nPeriod < chn.nPortamentoDest)
		{
			DoFreqSlide(chn, chn.nPeriod, actualDelta, true);
			if(chn.nPeriod >= chn.nPortamentoDest)
...
void CSoundFile::DoFreqSlide(ModChannel &chn, PitchT &period, PitchT amount, bool isTonePorta) const
{
	if(!period || !amount)
		return;
	MPT_ASSERT(!chn.HasCustomTuning());
	...
	{
		// IT Linear slides: continuous-math interpretation of the old linear-slide
		// tables, intentionally without the ignored-low-bits or minimum-step quirks.
		// period *= 2^(±amount/768)
		// 768 = 4 fine units * 12 semitones * 16 steps/semitone
		// kPeriodsAreHertz inverts direction: frequency up = period down
		if(PeriodsAreFrequencies())
			period *= std::pow(2.0, amount / 768.0);
		else
			period *= std::pow(2.0, -amount / 768.0);
	} else if(!m_SongFlags[SONG_LINEARSLIDES] && m_playBehaviour[kPeriodsAreHertz])
	{
		...
		// the original pitch deviation.
		if(GetType() != MOD_TYPE_XM
		   && GetType() != MOD_TYPE_MOD
		   && chn.pModSample && chn.pModSample->nC5SpeedOriginal != 0
		   && chn.pModSample->nC5SpeedOriginal != chn.pModSample->nC5Speed)
		{
			amount = amount * chn.pModSample->nC5SpeedOriginal / chn.pModSample->nC5Speed;
		}
		constexpr double k = 1712.0 * 8363.0;
		if(amount < 0)
		{
			// Go down
			period = k * period / (period * (-amount) + k);
		} else if(amount > 0)
		{
			// Go up
			const double periodDiv = k - period * amount;
			if(periodDiv <= 0)
			{
				if(isTonePorta)
				{
					period = static_cast<PitchT>(int32_max);
					return;
				} else
				{
					period = 0;
			...
			period = k * period / periodDiv;
		}
	} else
	{
		// C5Speed-based slides need amount scaling after sample-rate replacement.
		// XM replacement keeps the original runtime note math and applies the
		// sample-rate ratio in GetChannelIncrement(), so raw period deltas and
		// vibrato stay in their original FT2 units here.
		// MOD uses fixed period tables unrelated to c5speed; its pitch correction
		// is a post-hoc frequency scale applied in GetChannelIncrement() that
		// cancels in ratios, so no slide compensation is needed.
		if(GetType() != MOD_TYPE_XM
		   && GetType() != MOD_TYPE_MOD
		   && chn.pModSample && chn.pModSample->nC5SpeedOriginal != 0
		   && chn.pModSample->nC5SpeedOriginal != chn.pModSample->nC5Speed)
...
PitchT CSoundFile::GetNoteFromPeriod(PitchT period, int32 nFineTune, FreqT nC5Speed) const
{
	if(!period) return 0;
	...
	while(count > 0)
	{
		...
		PitchT n = GetPeriodFromNote(midNote, nFineTune, nC5Speed);
...
PitchT CSoundFile::GetPeriodFromNote(uint32 note, int32 nFineTune, FreqT nC5Speed) const
{
	...
	if(!UseFinetuneAndTranspose())
	{
		...
		if(PeriodsAreFrequencies())
		{
			...
			// note 0 = C-0. Middle C (C-5) = note 60. This is the closed form of the
			// old LinearSlideUpTable[(note % 12) * 16] lookup with octave scaling.
			double freq = nC5Speed * std::pow(2.0, (static_cast<int>(note) - 60) / 12.0);
			if(freq > static_cast<double>(int32_max)) freq = static_cast<double>(int32_max);
			return static_cast<PitchT>(freq);
		} else if(m_SongFlags[SONG_LINEARSLIDES])
		{
			return static_cast<PitchT>((FreqS3MTable[note % 12u] << 5) >> (note / 12));
		} else
		{
			//(a*b)/c
			double divisor = nC5Speed * (1u << (note / 12u));
			return static_cast<PitchT>(8363.0 * (FreqS3MTable[note % 12u] << 5) / divisor);
...
// Converts period value to sample frequency in Hz (double precision).
double CSoundFile::GetFreqFromPeriod(PitchT period, FreqT c5speed, PitchT nPeriodFrac) const
{
	...
	if ((GetType() & (MOD_TYPE_XM | MOD_TYPE_MTM)) || (m_SongFlags[SONG_LINEARSLIDES] && UseFinetuneAndTranspose()))
	{
		if(m_playBehaviour[kFT2Periods])
		{
			...
			period = static_cast<PitchT>(static_cast<uint32>(period) & 0xFFFF);
		}
		if(m_SongFlags[SONG_LINEARSLIDES])
		{
			if(m_playBehaviour[kFT2Periods])
			{
				// FT2 compatibility: must use integer table for exact wraparound emulation.
				// Test case: FreqWraparound.xm
				uint32 iPeriod = static_cast<uint32>(period);
				uint32 div = ((9216u + 767u - iPeriod) / 768);
				uint32 octave = ((14 - div) & 0x1F);
				// Preserve FT2's integer table math, then convert to Hz
				return static_cast<double>((XMLinearTable[iPeriod % 768] << (FREQ_FRACBITS + 2)) >> octave) / (1 << FREQ_FRACBITS);
			} else
			{
				// XM linear frequency: closed form of XMLinearTable[period % 768] with
				// octave shifting. period is in units of 1/768 octave.
	...
	} else if(GetType() == MOD_TYPE_669)
	{
		...
		return period + c5speed - 8363.0;
	} else if(GetType() == MOD_TYPE_MDL)
	{
		MPT_ASSERT(!PeriodsAreFrequencies());
		if(period > 1e8) period = 1e8;  // Overflow guard
		if (!c5speed) c5speed = 8363;
		double combinedPeriod = period * 256.0 + nPeriodFrac;
		return c5speed * (1712.0 * 128.0) / combinedPeriod;
	} else
	{
		if(period > 1e8) period = 1e8;  // Overflow guard
		if(PeriodsAreFrequencies())
		{
			...
			double combinedFreq = period * 256.0 + nPeriodFrac;
			return combinedFreq / 256.0;
		} else if(m_SongFlags[SONG_LINEARSLIDES] || GetType() == MOD_TYPE_DTM)
		{
			if(!c5speed)
				c5speed = 8363;
			double combinedPeriod = period * 256.0 + nPeriodFrac;
			return c5speed * (1712.0 * 256.0) / combinedPeriod;
		} else
		{
			double combinedPeriod = period * 256.0 + nPeriodFrac;
			return 8363.0 * (1712.0 * 256.0) / combinedPeriod;
		}
	}
}
