...
// Shared mip level count: 8 tables per mode.
// For the kernel-mip modes (Polyphase/SINC8/4-tap/Linear) the levels are
// octave-spaced over 1x–128x downsample and trilinear-blended via log2(ratio).
// For Aniso-64 the octave axis is handled by per-sample decimated DATA mips
// (SampleMipChain.cpp); its 8 kernel levels span a single octave at
// eighth-octave steps and are selected per slice by the residual ratio.
#define MIP_LEVELS 8
...
public:
	...
	double aniso64_k_r = 0.8;       // Pitch-axis footprint widening weight: R = 1 + k_r·|μ̇| (Eq. 15)
public:
...
public:
	...
	// Mip chains: Kaiser sinc tables generated by getsinc_n() at each mode's
	// native tap width. Kernel-mip modes: octave-spaced, log2(ratio) selects
	// adjacent levels, fractional part drives trilinear blend. Aniso-64:
	// fractional one-octave family matched to the residual ratio over the
	// per-sample data mip pyramid (see Tables.cpp / SampleMipChain.cpp).
