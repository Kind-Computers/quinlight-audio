...
void ModChannel::RecalcTuningFreq(Tuning::RATIOTYPE vibratoFactor, Tuning::NOTEINDEXTYPE arpeggioSteps, const CSoundFile &sndFile)
{
	...
	// Custom tuning: nPeriod stores frequency in Hz directly (double precision).
	// GetChannelIncrement reads this via `freq = chn.nPeriod` for custom tuning.
	nPeriod = static_cast<double>(nC5Speed) * vibratoFactor * pModInstrument->pTuning->GetRatio(static_cast<Tuning::NOTEINDEXTYPE>(note - NOTE_MIDDLEC + arpeggioSteps), nFineTune + m_PortamentoFineSteps);
}
