...
static void getsinc_n(SINC_TYPE *psinc, double beta, double cutoff, int width, int phases)
{
	if(cutoff >= 0.999)
	{
	...
	const double halfWidthSq = static_cast<double>(halfWidth) * static_cast<double>(halfWidth);
	const int total = width * phases;

	// Generate unnormalized coefficients into temp buffer
	std::vector<double> raw(total);
	for(int isrc = 0; isrc < total; isrc++)
	{
		double fsinc;
		int ix = (width - 1) - (isrc & (width - 1));
		ix = (ix * phases) + (isrc / width);
		if(ix == (halfWidth * phases))
		{
			fsinc = 1.0;
		} else
		{
			const double x = (double)(ix - (halfWidth * phases)) * (1.0 / phases);
			const double xPi = x * kPi;
			fsinc = std::sin(xPi) * Izero(beta * std::sqrt(1.0 - x * x / halfWidthSq)) / (izeroBeta * xPi); // Kaiser window
		}
		raw[isrc] = fsinc * cutoff;
	}
	...
	{
#ifdef MPT_INTMIXER
		*psinc++ = mpt::saturate_round<SINC_TYPE>(raw[isrc] * (1 << SINC_QUANTSHIFT));
#else
		*psinc++ = static_cast<SINC_TYPE>(raw[isrc]);
#endif
	}
}
...
{
	getsinc_n(psinc, beta, cutoff, SINC_WIDTH, SINC_PHASES);
}


#ifdef MODPLUG_TRACKER
bool CResampler::StaticTablesInitialized = false;
Paula::BlepTables CResampler::blepTables;
SINC_TYPE CResampler::gAniso64Mip[MIP_LEVELS][SINC_PHASES_64 * SINC_WIDTH_64];
SINC_TYPE CResampler::gPolyphaseMip[MIP_LEVELS][SINC_PHASES * SINC_WIDTH];
SINC_TYPE CResampler::gSinc8Mip[MIP_LEVELS][SINC8_PHASES * SINC8_WIDTH];
SINC_TYPE CResampler::g4TapMip[MIP_LEVELS][TAP4_PHASES * TAP4_WIDTH];
SINC_TYPE CResampler::gLinearMip[MIP_LEVELS][TAP2_PHASES * TAP2_WIDTH];
#endif // MODPLUG_TRACKER
...
void CResampler::InitializeTablesFromScratch(bool force)
{
	...
	MPT_MAYBE_CONSTANT_IF(initParameterIndependentTables)
	{
		...
		// Polyphase mip chain (16-tap, 16384 phases)
		{
			static const MipParam poly[MIP_LEVELS] = {
				{ 0.97,  13.0 },   // Level 0: near-unity
				{ 0.46,  11.0 },   // Level 1: 2x
				{ 0.23,  10.0 },   // Level 2: 4x
				{ 0.115,  9.0 },   // Level 3: 8x
				{ 0.058,  8.0 },   // Level 4: 16x
				{ 0.029,  7.0 },   // Level 5: 32x
				{ 0.015,  6.5 },   // Level 6: 64x
				{ 0.008,  6.0 },   // Level 7: 128x
			};
			for(int i = 0; i < MIP_LEVELS; i++)
				getsinc_n(gPolyphaseMip[i], poly[i].beta, poly[i].cutoff, SINC_WIDTH, SINC_PHASES);
		}

		// SINC8 mip chain (8-tap, 4096 phases)
		{
			static const MipParam sinc8[MIP_LEVELS] = {
				{ 0.90,  9.0 },
				{ 0.43,  8.0 },
				{ 0.21,  7.0 },
				{ 0.105, 6.5 },
			...
			for(int i = 0; i < MIP_LEVELS; i++)
				getsinc_n(gSinc8Mip[i], sinc8[i].beta, sinc8[i].cutoff, SINC8_WIDTH, SINC8_PHASES);
		}

		// 4-tap mip chain (shared: Cubic + Catmull-Rom, 4096 phases)
		{
			static const MipParam tap4[MIP_LEVELS] = {
				{ 0.85,  6.0 },
				{ 0.40,  5.5 },
				{ 0.20,  5.0 },
				{ 0.10,  4.5 },
			...
			for(int i = 0; i < MIP_LEVELS; i++)
				getsinc_n(g4TapMip[i], tap4[i].beta, tap4[i].cutoff, TAP4_WIDTH, TAP4_PHASES);
		}

		// 2-tap mip chain (Linear, 4096 phases)
		{
			static const MipParam tap2[MIP_LEVELS] = {
				{ 0.75,  3.0 },
				{ 0.35,  2.5 },
				{ 0.17,  2.0 },
				{ 0.085, 1.5 },
			...
			for(int i = 0; i < MIP_LEVELS; i++)
				getsinc_n(gLinearMip[i], tap2[i].beta, tap2[i].cutoff, TAP2_WIDTH, TAP2_PHASES);
		}

		// Runtime ISA detection for SIMD-accelerated 64-tap dot products
		// Cascade: SSE2 (baseline x64) → AVX → AVX2+FMA3 → AVX-512F
		m_hasAVX = false;
		m_hasAVX2 = false;
		m_hasAVX512 = false;
...
#ifdef __GNUC__
		{
			...
			// CPUID(1,0): AVX, OSXSAVE, FMA3
			__asm__ __volatile__("cpuid" : "=a"(eax), "=b"(ebx), "=c"(ecx), "=d"(edx) : "a"(1), "c"(0));
			bool hasFMA3    = (ecx & (1u << 12)) != 0;
			bool hasOSXSAVE = (ecx & (1u << 27)) != 0;
			bool hasAVX     = (ecx & (1u << 28)) != 0;

			// AVX requires OS support (OSXSAVE)
			m_hasAVX = hasAVX && hasOSXSAVE;

			if(m_hasAVX)
			{
				// CPUID(7,0): AVX2, AVX-512F
				__asm__ __volatile__("cpuid" : "=a"(eax), "=b"(ebx), "=c"(ecx), "=d"(edx) : "a"(7), "c"(0));
				bool hasAVX2    = (ebx & (1u << 5)) != 0;
				bool hasAVX512F = (ebx & (1u << 16)) != 0;

				// AVX2+FMA3: required for fused multiply-add path
				m_hasAVX2 = hasAVX2 && hasFMA3;

				// AVX-512: requires AVX2 + OS extended state for ZMM registers
				if(m_hasAVX2 && hasAVX512F)
				{
					unsigned int xcr0_lo, xcr0_hi;
					__asm__ __volatile__("xgetbv" : "=a"(xcr0_lo), "=d"(xcr0_hi) : "c"(0));
					// XCR0 bits [7:5]: OPMASK + ZMM_Hi256 + Hi16_ZMM
					m_hasAVX512 = (xcr0_lo & 0xE0) == 0xE0;
				}
			}
		}
#endif // __GNUC__
...
void CResampler::InitializeTablesFromCache()
{
	...
	for(int i = 0; i < MIP_LEVELS; i++)
	{
		std::copy(s_CachedResampler.gAniso64Mip[i], s_CachedResampler.gAniso64Mip[i] + SINC_PHASES_64*SINC_WIDTH_64, gAniso64Mip[i]);
		std::copy(s_CachedResampler.gPolyphaseMip[i], s_CachedResampler.gPolyphaseMip[i] + SINC_PHASES*SINC_WIDTH, gPolyphaseMip[i]);
		std::copy(s_CachedResampler.gSinc8Mip[i], s_CachedResampler.gSinc8Mip[i] + SINC8_PHASES*SINC8_WIDTH, gSinc8Mip[i]);
		std::copy(s_CachedResampler.g4TapMip[i], s_CachedResampler.g4TapMip[i] + TAP4_PHASES*TAP4_WIDTH, g4TapMip[i]);
		std::copy(s_CachedResampler.gLinearMip[i], s_CachedResampler.gLinearMip[i] + TAP2_PHASES*TAP2_WIDTH, gLinearMip[i]);
	}
	m_hasAVX = s_CachedResampler.m_hasAVX;
	m_hasAVX2 = s_CachedResampler.m_hasAVX2;
	m_hasAVX512 = s_CachedResampler.m_hasAVX512;
