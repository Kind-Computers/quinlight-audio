...
// Pitch control plane types. Currently int32/uint32 for identical behavior;
// will be flipped to double for higher-precision pitch/frequency arithmetic.
using PitchT = double;  // Period values, slide amounts, portamento targets
using FreqT  = double;  // Sample rates (C5Speed) in Hz

// Volume and pan processing types. Double precision eliminates cascading
// integer division truncation through the volume/pan processing chains.
using VolT = double;  // Computed volume pipeline (vol, insVol, envelope products)
using PanT = double;  // Pan processing (nRealPan, pan envelope, panbrello)
...
inline std::array<ResamplingMode, 7> AllModes() noexcept { return { { SRCMODE_NEAREST, SRCMODE_LINEAR, SRCMODE_CUBIC, SRCMODE_SINC8, SRCMODE_SINC8LP, SRCMODE_ANISO64, SRCMODE_CATMULL } }; }

inline std::array<ResamplingMode, 8> AllModesWithDefault() noexcept { return { { SRCMODE_NEAREST, SRCMODE_LINEAR, SRCMODE_CUBIC, SRCMODE_SINC8, SRCMODE_SINC8LP, SRCMODE_ANISO64, SRCMODE_CATMULL, SRCMODE_DEFAULT } }; }

constexpr ResamplingMode Default() noexcept { return SRCMODE_ANISO64; }

constexpr bool IsKnownMode(int mode) noexcept { return (mode >= 0 && mode < SRCMODE_DEFAULT) || (mode == SRCMODE_ANISO64) || (mode == SRCMODE_CATMULL); }
...
constexpr int Length(ResamplingMode mode) noexcept
{
	return mode == SRCMODE_NEAREST ? 1
		...
		: mode == SRCMODE_CATMULL ? 4
		: mode == SRCMODE_SINC8 ? 8
		: mode == SRCMODE_SINC8LP ? 16
		: mode == SRCMODE_ANISO64 ? 64
		: 0;
}

constexpr bool HasAA(ResamplingMode mode) noexcept { return mode != SRCMODE_NEAREST && mode != SRCMODE_AMIGA; }

constexpr ResamplingMode AddAA(ResamplingMode mode) noexcept { return mode; }

constexpr ResamplingMode RemoveAA(ResamplingMode mode) noexcept { return mode; }
