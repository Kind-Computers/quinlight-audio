...
static PitchT ScaleXMVibratoAmount(const CSoundFile &, ModChannel &, PitchT amount)
{
	return amount;
}
...
void CSoundFile::InitPlayer(bool bReset)
{
	...
	m_Resampler.UpdateTables();
#if !defined(NO_REVERB) && defined(MPT_INTMIXER)
...
bool CSoundFile::FadeSong(uint32 msec)
{
	...
	for (uint32 noff=0; noff < m_nMixChannels; noff++)
	{
		...
		pramp.leftRamp = -pramp.leftVol / nRampLength;
		pramp.rightRamp = -pramp.rightVol / nRampLength;
		pramp.rampLeftVol = pramp.leftVol;
		pramp.rampRightVol = pramp.rightVol;
		pramp.nRampLength = nRampLength;
		pramp.nRampPosition = 0;
...
#else
	const double normalize_factor = 0.5; // cumulative mid/side normalization factor (1/sqrt(2))*(1/sqrt(2))
	const double factor = static_cast<double>(separation) / static_cast<double>(MixerSettings::StereoSeparationScale); // sep / 128
	const double mid_factor = normalize_factor;
	const double side_factor = factor * normalize_factor;
#endif
...
void CSoundFile::ProcessDSP(uint32 countChunk)
{
	...
	// AGC works with both int and float mixer paths
	#ifndef NO_AGC
...
void CSoundFile::ProcessVolumeSwing(ModChannel &chn, VolT &vol) const
{
...
void CSoundFile::ProcessTremolo(ModChannel &chn, VolT &vol) const
{
...
void CSoundFile::ProcessTremor(CHANNELINDEX nChn, VolT &vol)
{
...
void CSoundFile::ProcessVolumeEnvelope(ModChannel &chn, VolT &vol) const
{
	if(IsEnvelopeProcessed(chn, ENV_VOLUME))
	{
		...
		// Get values in [0, 256]
		double envval = pIns->VolEnv.GetValueFromPositionF(envpos, 256.0);
		...
		{
			double envValueAtReleaseJump = chn.VolEnv.nEnvValueAtReleaseJump;
			double envValueAtReleaseNode = pIns->VolEnv[pIns->VolEnv.nReleaseNode].value * 4.0;
			...
			if(m_playBehaviour[kLegacyReleaseNode])
			{
				...
				double relativeVolumeChange = (envval - envValueAtReleaseNode) * 2.0;
		...
		vol = vol * Clamp(envval, 0.0, 512.0) / 256.0;
	}
...
void CSoundFile::ProcessPanningEnvelope(ModChannel &chn) const
{
	if(IsEnvelopeProcessed(chn, ENV_PANNING))
	{
		...
		// Get values in [-32, 32]
		const double envval = pIns->PanEnv.GetValueFromPositionF(envpos, 64.0) - 32.0;

		PanT pan = chn.nRealPan;
		if(pan >= 128)
		{
			pan += (envval * (256.0 - pan)) / 32.0;
		} else
		{
			pan += (envval * pan) / 32.0;
		}
		chn.nRealPan = Clamp(pan, 0.0, 256.0);
	}
}


int CSoundFile::ProcessPitchFilterEnvelope(ModChannel &chn, PitchT &period) const
{
	if(IsEnvelopeProcessed(chn, ENV_PITCH))
	{
		...
		} else
		{
			...
			{
				// Pitch envelope: envval indexes into LinearSlideUpTable where each
				// entry encodes 2^(n/192) — one step = 1/192 octave (the regular
				// slide scale, not the 1/768 fine-slide scale used by DoFreqSlide).
				const double sign = PeriodsAreFrequencies() ? 1.0 : -1.0;
				period *= std::pow(2.0, sign * envval / 192.0);
...
void CSoundFile::ProcessInstrumentFade(ModChannel &chn, VolT &vol) const
{
	// FadeOut volume
...
void CSoundFile::ProcessPitchPanSeparation(PanT &pan, int note, const ModInstrument &instr)
{
...
void CSoundFile::ProcessPanbrello(ModChannel &chn) const
{
	...
	if(chn.rowCommand.command == CMD_PANBRELLO)
	{
		...
		// IT compatibility: IT has its own, more precise tables
		if(m_playBehaviour[kITVibratoTremoloPanbrello])
			panpos = static_cast<uint32>(chn.nPanbrelloPos);
		else
			panpos = ((static_cast<uint32>(chn.nPanbrelloPos) + 0x10) >> 2);
	...
	if(pdelta)
	{
		...
		PanT panResult = pdelta + chn.nRealPan;
		chn.nRealPan = Clamp(panResult, 0.0, 256.0);
	}
}


void CSoundFile::ProcessArpeggio(CHANNELINDEX nChn, PitchT &period, Tuning::NOTEINDEXTYPE &arpeggioSteps)
{
	ModChannel &chn = m_PlayState.Chn[nChn];
	...
	if(chn.nCommand == CMD_ARPEGGIO)
	{
		...
		} else
		{
			...
			if(m_playBehaviour[kITArpeggio])
			{
				...
				if(chn.nArpeggio != 0)
				{
					uint32 arpSemitones = 0;
					switch(tick % 3)
					{
					case 1: arpSemitones = (chn.nArpeggio >> 4); break;
					case 2: arpSemitones = (chn.nArpeggio & 0x0F); break;
					}
					if(arpSemitones != 0)
					{
						// IT arpeggio used LinearSlideUpTable[(semi) * 16]: 16 linear-slide
						// steps are exactly one semitone, so this closes to 2^(semi/12).
						double arpFactor = std::pow(2.0, arpSemitones / 12.0);
			...
			} else if(m_playBehaviour[kFT2Arpeggio])
			{
				...
				if(!m_PlayState.m_flags[SONG_FIRSTTICK])
				{
					...
					if(arpPos != 0)
					{
						...
						{
							period = std::max(period, GetPeriodFromNote(108 + NOTE_MIN, 0, chn.nC5Speed));
						}
					}
				}
			}
...
void CSoundFile::ProcessVibrato(CHANNELINDEX nChn, PitchT &period, Tuning::RATIOTYPE &vibratoFactor)
{
	ModChannel &chn = m_PlayState.Chn[nChn];
	...
	if(chn.dwFlags[CHN_VIBRATO])
	{
		...
		{
			if(static_cast<uint32>(chn.nVibratoPos) % 2u)
			{
		...
		if(chn.HasCustomTuning())
		{
			...
			vibratoFactor += 0.05 * static_cast<double>(vdelta * static_cast<int>(chn.nVibratoDepth)) / (128.0 * 60.0);
			chn.m_CalculateFreq = true;
		...
		} else
		{
			...
			const PitchT slide = ScaleXMVibratoAmount(*this, chn, vdelta);

			DoFreqSlide(chn, period, slide);
			...
			if(plugin != nullptr)
			{
				...
				plugin->MidiVibrato(mpt::saturate_round<int32>(slide), pwd, nChn);
			}
#endif // NO_PLUGINS
		}
...
void CSoundFile::ProcessSampleAutoVibrato(ModChannel &chn, PitchT &period, Tuning::RATIOTYPE &vibratoFactor, int &nPeriodFrac) const
{
	// Sample Auto-Vibrato
	if(chn.pModSample != nullptr && chn.pModSample->nVibDepth)
	{
		...
		if(m_playBehaviour[kITVibratoTremoloPanbrello] && !hasTuning && GetType() != MOD_TYPE_MT2)
		{
			...
			*/
			const int vibpos = static_cast<int>(chn.nAutoVibPos) & 0xFF;
			double adepth = chn.nAutoVibDepth; // (1)
			adepth += pSmp->nVibSweep; // (2 & 3)
			if(adepth > pSmp->nVibDepth * 256.0) adepth = pSmp->nVibDepth * 256.0;
			...
			// IT auto-vibrato historically used coarse LinearSlide tables at l / 4 plus
			// FineLinearSlide tables for l & 3. One signed vdelta unit is therefore
			// 1/4 of a regular linear-slide step = 1/768 octave.
			const double sign = PeriodsAreFrequencies() ? 1.0 : -1.0;
			period *= std::pow(2.0, sign * vdelta / 768.0);
			nPeriodFrac = 0;
		} else
		{
			// MPT's autovibrato code
			double autoVibDepth = chn.nAutoVibDepth;
			const double fullDepth = pSmp->nVibDepth * 256.0;
			...
			} else
			{
				...
				{
					autoVibDepth += pSmp->nVibSweep * 2.0;
					if(autoVibDepth > fullDepth) autoVibDepth = fullDepth;
			...
			{
			case VIB_RANDOM:
				vdelta = ModRandomTable[static_cast<int>(chn.nAutoVibPos) & 0x3F];
				chn.nAutoVibPos++;
				break;
			case VIB_RAMP_DOWN:
				vdelta = ((0x40 - (static_cast<uint32>(chn.nAutoVibPos) / 2u)) & 0x7F) - 0x40;
				break;
			case VIB_RAMP_UP:
				vdelta = ((0x40 + (static_cast<uint32>(chn.nAutoVibPos) / 2u)) & 0x7F) - 0x40;
				break;
			case VIB_SQUARE:
				vdelta = (static_cast<int>(chn.nAutoVibPos) & 128) ? +64 : -64;
				break;
			case VIB_SINE:
			default:
				if(GetType() != MOD_TYPE_MT2)
				{
					vdelta = -ITSinusTable[static_cast<int>(chn.nAutoVibPos) & 0xFF];
				} else
				{
					...
					vdelta = (-ITSinusTable[(static_cast<int>(chn.nAutoVibPos) + 192) & 0xFF] + 64) / 2;
				}
			}
			int n = static_cast<int>((vdelta * autoVibDepth) / 256.0);

			if(hasTuning)
			{
				//Vib sweep is not taken into account here.
				vibratoFactor += 0.05 * static_cast<double>(static_cast<int>(pSmp->nVibDepth) * vdelta) / 4096.0; //4096 == 64^2
				//See vibrato for explanation.
				chn.m_CalculateFreq = true;
				/*
				Finestep vibrato:
				const double autoVibDepth = pSmp->nVibDepth * val / 4096.0; //4096 == 64^2
			...
			else //Original behavior
			{
				if (GetType() != MOD_TYPE_XM)
				{
					// Legacy MPT auto-vibrato interpolated between LinearSlide table
					// entries: n has 256 substeps per 1/192-octave slide step.
					const double sign = PeriodsAreFrequencies() ? 1.0 : -1.0;
					period *= std::pow(2.0, sign * n / (256.0 * 192.0));
					nPeriodFrac = 0;
				} else
				{
					period += (n / 64);
...
void CSoundFile::ProcessRamping(ModChannel &chn) const
{
	...
	if(chn.dwFlags[CHN_VOLUMERAMP] && (chn.leftVol != chn.newLeftVol || chn.rightVol != chn.newRightVol))
	{
		...
		if(!enableCustomRamp)
		{
			if((chn.leftVol != 0 || chn.rightVol != 0) && (chn.newLeftVol != 0 || chn.newRightVol != 0) && !chn.dwFlags[CHN_FASTVOLRAMP])
			{
		...
		chn.leftRamp = (chn.newLeftVol - chn.leftVol) / rampLength;
		chn.rightRamp = (chn.newRightVol - chn.rightVol) / rampLength;
		// Back-calculate start volume so that leftVol + rampLength * leftRamp == newLeftVol exactly
		chn.leftVol = chn.newLeftVol - chn.leftRamp * rampLength;
		chn.rightVol = chn.newRightVol - chn.rightRamp * rampLength;

		if (chn.leftRamp != 0 || chn.rightRamp != 0)
		{
...
// Returns channel increment and frequency in Hz (double precision).
std::pair<SamplePosition, double> CSoundFile::GetChannelIncrement(const ModChannel &chn, PitchT period, int periodFrac) const
{
	double freq;
	if(!chn.HasCustomTuning())
	{
		...
		// MOD period tables use the PAL Amiga clock and ignore nC5Speed, so they
		// need the same post-hoc scaling.
	...
	if(int32 finetune = chn.microTuning; finetune != 0)
	{
		...
		if(finetune)
			// microTuning is in 1/128 of a semitone, scaled by the instrument PWD in semitones.
			freq = freq * std::pow(2.0, finetune / (12.0 * 256.0 * 128.0));
	}

	// Applying Pitch/Tempo lock
	if(ins && ins->pitchToTempoLock.GetRaw())
	{
		freq = freq * m_PlayState.m_nMusicTempo.GetRaw() / ins->pitchToTempoLock.GetRaw();
	}

	// Clamp to reasonable range to avoid SamplePosition overflow
	if(freq > 1e9) freq = 1e9;
	if(freq < 0.0) freq = 0.0;
	return {SamplePosition::FromDouble(freq / m_MixerSettings.gdwMixingFreq), freq};
}
...
bool CSoundFile::ReadNote()
{
	...
	for(CHANNELINDEX nChn = 0; nChn < m_PlayState.Chn.size(); nChn++)
	{
		...
		if(chn.dwFlags[CHN_NOTEFADE] && (chn.nFadeOutVol == 0 && chn.leftVol == 0 && chn.rightVol == 0) && !m_playBehaviour[kFT2ProcessSilentChannels])
		{
			chn.nLength = 0;
		...
		if(chn.dwFlags[CHN_MUTE] || (nChn >= GetNumChannels() && !chn.nLength))
		{
			...
			chn.nLeftVU = chn.nRightVU = 0.0;
			continue;
		}
		// Save previous increment for anisotropic β-shear (must happen BEFORE the
		// increment is zeroed — the old increment is still live from the previous tick).
		chn.prevDeltaIncrement = chn.increment - chn.prevIncrement;
		...
		{
			VolT vol = chn.nVolume;
			VolT insVol = chn.nInsVol;		// This is the "SV * IV" value in ITTECH.TXT
			...
			// vol is 14-bits (continuous with VolT=double)
			if (vol)
			{
				...
				} else if (m_PlayConfig.getGlobalVolumeAppliesToMaster())
				{
					...
					chn.nRealVolume = (vol * MAX_GLOBAL_VOLUME) * (chn.nGlobalVol * insVol) / static_cast<double>(1 << 20);
				} else
				{
					chn.nRealVolume = (vol * m_PlayState.m_nGlobalVolume) * (chn.nGlobalVol * insVol) / static_cast<double>(1 << 20);
				}
			}
		...
		if(samplePlaying)
		{
			...
			if((chn.dwFlags & (CHN_ADLIB | CHN_MUTE | CHN_SYNCMUTE)) == CHN_ADLIB && m_opl)
			{
				...
				if(doProcess && !(GetType() == MOD_TYPE_S3M && chn.dwFlags[CHN_KEYOFF]))
				{
					...
					auto milliHertz = static_cast<uint32>(freq * 261625.0 / 8363.0);
#ifndef MODPLUG_TRACKER
				...
				if(doProcess)
				{
					...
					m_opl->Volume(nChn, static_cast<uint8>(Util::muldivr_unsigned(static_cast<uint32>(chn.nCalcVolume) * chn.nGlobalVol * chn.nInsVol, 63, 1 << 26)), false);
					chn.nRealPan = m_opl->Pan(nChn, static_cast<int32>(chn.nRealPan)) * 128 + 128;
				}
		...
		// Volume ramping
		chn.dwFlags.set(CHN_VOLUMERAMP, (chn.nRealVolume != 0 || chn.rightVol != 0 || chn.leftVol != 0) && !chn.dwFlags[CHN_ADLIB]);

		constexpr double VUMETER_DECAY = 4.0 / 128.0;  // Equivalent to old uint8 decay of 4 per tick
		chn.nLeftVU = (chn.nLeftVU > VUMETER_DECAY) ? (chn.nLeftVU - VUMETER_DECAY) : 0.0;
		chn.nRightVU = (chn.nRightVU > VUMETER_DECAY) ? (chn.nRightVU - VUMETER_DECAY) : 0.0;
		...
		{
			// Update VU-Meter — normalized [0.0, 1.0]
			double vul = std::min(chn.nRealVolume * (256.0 - chn.nRealPan) / (256.0 * 256.0), 1.0);
			if (chn.nLeftVU > 1.0) chn.nLeftVU = vul;  // Sentinel: snap to current level on note trigger
			vul *= 0.5;
			if (chn.nLeftVU < vul) chn.nLeftVU = vul;
			double vur = std::min(chn.nRealVolume * chn.nRealPan / (256.0 * 256.0), 1.0);
			if (chn.nRightVU > 1.0) chn.nRightVU = vur;
			vur *= 0.5;
			if (chn.nRightVU < vur) chn.nRightVU = vur;
		} else
		{
			// Note change but no sample
			if (chn.nLeftVU > 1.0) chn.nLeftVU = 0.0;
			if (chn.nRightVU > 1.0) chn.nRightVU = 0.0;
		}
		...
		if (chn.pCurrentSample)
		{
			...
			// Adjusting volumes
			{
				PanT pan = (m_MixerSettings.gnChannels >= 2) ? Clamp(chn.nRealPan, 0, 256) : 128;

				double realvol = static_cast<double>(chn.nRealVolume * kChnMasterVol) / 128.0;
				...
				if(panningMode == PanningMode::SoftPanning || (panningMode == PanningMode::Undetermined && (m_MixerSettings.MixerFlags & SNDMIX_SOFTPANNING)))
				{
					if(pan < 128)
					{
						chn.newLeftVol = realvol * 128.0 / 256.0;
						chn.newRightVol = realvol * pan / 256.0;
					} else
					{
						chn.newLeftVol = realvol * (256 - pan) / 256.0;
						chn.newRightVol = realvol * 128.0 / 256.0;
					}
				} else if(panningMode == PanningMode::FT2Panning)
				{
					...
					LimitMax(pan, static_cast<PanT>(255));

					const int panIdx = static_cast<int>(pan);
					const double panL = (panIdx > 0 ? XMPanningTable[256 - panIdx] : 65536) / 65536.0;
					const double panR = XMPanningTable[panIdx] / 65536.0;
					chn.newLeftVol = realvol * panL;
					chn.newRightVol = realvol * panR;
				} else
				{
					chn.newLeftVol = realvol * (256 - pan) / 256.0;
					chn.newRightVol = realvol * pan / 256.0;
...
#else
template<int channels>
MPT_FORCEINLINE void ApplyGlobalVolumeWithRamping(mixsample_t *SoundBuffer, mixsample_t *RearBuffer, uint32 lCount, int32 m_nGlobalVolume, int32 step, int32 &m_nSamplesToGlobalVolRampDest, int32 &m_lHighResRampingGlobalVolume)
{
	constexpr bool isStereo = (channels >= 2);
	constexpr bool hasRear = (channels >= 4);
	const mixsample_t maxGlobalVolF = static_cast<mixsample_t>(MAX_GLOBAL_VOLUME);
	const mixsample_t rampScale = static_cast<mixsample_t>(1) / static_cast<mixsample_t>(MAX_GLOBAL_VOLUME << VOLUMERAMPPRECISION);
	for(uint32 pos = 0; pos < lCount; ++pos)
	{
		if(m_nSamplesToGlobalVolRampDest > 0)
		{
			m_lHighResRampingGlobalVolume += step;
			const mixsample_t gain = static_cast<mixsample_t>(m_lHighResRampingGlobalVolume) * rampScale;
			                       SoundBuffer[0] *= gain;
			if constexpr(isStereo) SoundBuffer[1] *= gain;
			if constexpr(hasRear)  RearBuffer[0]  *= gain; else MPT_UNUSED_VARIABLE(RearBuffer);
			if constexpr(hasRear)  RearBuffer[1]  *= gain; else MPT_UNUSED_VARIABLE(RearBuffer);
			m_nSamplesToGlobalVolRampDest--;
		} else
		{
			const mixsample_t gain = static_cast<mixsample_t>(m_nGlobalVolume) / maxGlobalVolF;
			                       SoundBuffer[0] *= gain;
			if constexpr(isStereo) SoundBuffer[1] *= gain;
			if constexpr(hasRear)  RearBuffer[0]  *= gain; else MPT_UNUSED_VARIABLE(RearBuffer);
			if constexpr(hasRear)  RearBuffer[1]  *= gain; else MPT_UNUSED_VARIABLE(RearBuffer);
