...
bool ModSample::CopyWaveform(const ModSample &smpFrom)
{
	...
	runtimeFormat = smpFrom.runtimeFormat;
	saveBitsPerSample = smpFrom.saveBitsPerSample;
	if(AllocateSample())
	{
...
bool ModSample::ConvertStoredDataToFloat()
{
	if(!HasSampleData()
		|| GetRuntimeSampleFormat() == RuntimeSampleFormat::Float32
		|| GetRuntimeSampleFormat() == RuntimeSampleFormat::Float64)
	{
		return true;
	}

	const RuntimeSampleFormat oldFormat = GetRuntimeSampleFormat();
	const uint8 numChannels = GetNumChannels();
	void *newWaveform = AllocateSample(nLength, sizeof(double) * numChannels);
	if(newWaveform == nullptr)
	{
		return false;
	}

	const SmpLength count = nLength * numChannels;
	double *out = static_cast<double *>(newWaveform);
	if(oldFormat == RuntimeSampleFormat::Int16)
	{
		const int16 *in = sample16();
		for(SmpLength i = 0; i < count; i++)
		{
			out[i] = static_cast<double>(in[i]) * (1.0 / 32768.0);
		}
		if(GetSaveBitsPerSample() < 16)
		{
			SetSaveBitsPerSample(16);
		}
	} else if(oldFormat == RuntimeSampleFormat::Float32)
	{
		const somefloat32 *in = samplef();
		for(SmpLength i = 0; i < count; i++)
		{
	...
	{
		const int8 *in = sample8();
		for(SmpLength i = 0; i < count; i++)
		{
			out[i] = static_cast<double>(in[i]) * (1.0 / 128.0);
		}
	}

	void *oldWaveform = samplev();
	runtimeFormat = RuntimeSampleFormat::Float64;
	pData.pSample = newWaveform;
	FreeSample(oldWaveform);
	return true;
}
...
somefloat32 ModSample::ReadSampleAsFloat(SmpLength frame, uint8 channel) const noexcept
{
	if(!HasSampleData() || frame >= nLength)
	{
		return 0.0f;
	}
	const uint8 numChannels = GetNumChannels();
	...
	const SmpLength index = frame * numChannels + channel;
	switch(GetRuntimeSampleFormat())
	{
	case RuntimeSampleFormat::Float64:
		return static_cast<somefloat32>(sampled()[index]);
	case RuntimeSampleFormat::Float32:
		return samplef()[index];
	case RuntimeSampleFormat::Int16:
		return static_cast<somefloat32>(sample16()[index]) * (1.0f / 32768.0f);
	case RuntimeSampleFormat::Int8:
	case RuntimeSampleFormat::Auto:
	default:
		return static_cast<somefloat32>(sample8()[index]) * (1.0f / 128.0f);
	}
}


int8 ModSample::ReadSampleAsInt8(SmpLength frame, uint8 channel) const noexcept
{
	if(!HasSampleData() || frame >= nLength)
	{
		return 0;
	...
	case RuntimeSampleFormat::Float64:
	{
		double val = sampled()[index];
		if(val > (127.0 / 128.0))
			val = 127.0 / 128.0;
		if(val < -1.0)
			val = -1.0;
		return (val >= 0.0)
			? mpt::saturate_round<int8>(static_cast<somefloat32>(val * 127.0))
			: mpt::saturate_round<int8>(static_cast<somefloat32>(val * 128.0));
	}
	case RuntimeSampleFormat::Float32:
	{
		somefloat32 val = samplef()[index];
		if(val > (127.0f / 128.0f))
			val = 127.0f / 128.0f;
		if(val < -1.0f)
			val = -1.0f;
		return (val >= 0.0f)
			? mpt::saturate_round<int8>(val * 127.0f)
			: mpt::saturate_round<int8>(val * 128.0f);
	}
	case RuntimeSampleFormat::Int16:
		return static_cast<int8>(mpt::rshift_signed(sample16()[index], 8));
	case RuntimeSampleFormat::Int8:
	case RuntimeSampleFormat::Auto:
	default:
		return sample8()[index];
	}
}
...
void ModSample::PrecomputeLoops(CSoundFile &sndFile, bool updateChannels)
{
	...
	else if(GetRuntimeSampleFormat() == RuntimeSampleFormat::Float64)
