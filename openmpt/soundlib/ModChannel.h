...
struct ModChannel
{
	...
	double leftRamp;             // Per-sample ramping delta
	double rightRamp;            // Ditto
	double rampLeftVol;          // Current ramping volume
	double rampRightVol;         // Ditto
	...
	double newLeftVol, newRightVol;
	double nRealVolume;
	PanT nRealPan;
	int32 nVolume, nPan;
	double nFadeOutVol;
	PitchT nPeriod;  // Frequency in Hz if CSoundFile::PeriodsAreFrequencies() or using custom tuning, 4x Amiga periods otherwise
	FreqT nC5Speed;
	PitchT nPortamentoDest;
	PitchT cachedPeriod, glissandoPeriod;
	VolT nCalcVolume;                  // Calculated channel volume, 14-Bit (without global volume, pre-amp etc applied) - for MIDI macros
	EnvInfo VolEnv, PanEnv, PitchEnv;  // Envelope playback info
	double nAutoVibDepth;
	...
	double nAutoVibPos = 0.0, nVibratoPos = 0.0, nTremoloPos = 0.0, nPanbrelloPos = 0.0;
	...
	double nLeftVU = 0.0, nRightVU = 0.0;  // Normalized VU meters [0.0, 1.0]; values > 1.0 are note-trigger sentinels
	uint8 nActiveMacro;
