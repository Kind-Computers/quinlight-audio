...
struct InstrumentSynth::States::State
{
	...
	void ApplyChannelState(ModChannel &chn, PitchT &period, const CSoundFile &sndFile);
...
static PitchT ApplyLinearPitchSlide(PitchT target, const int32 totalAmount, const bool periodsAreFrequencies)
{
	...
	while(value > 0)
	{
		...
		target *= static_cast<PitchT>(table[amount]) / 65536.0;
		value -= amount;
	}
	return target;
}

static double ApplyPitchEnvelopeFactor(const int envval, const bool periodsAreFrequencies)
{
	const double sign = periodsAreFrequencies ? 1.0 : -1.0;
	return std::pow(2.0, sign * envval / 192.0);
}

static double ApplyPitchEnvelopeFactorReference(const int envval, const bool periodsAreFrequencies)
{
	const auto &upTable = periodsAreFrequencies ? LinearSlideUpTable : LinearSlideDownTable;
	const auto &downTable = periodsAreFrequencies ? LinearSlideDownTable : LinearSlideUpTable;
	const uint32 amount = std::min(static_cast<uint32>(std::abs(envval)), static_cast<uint32>(std::size(LinearSlideUpTable) - 1));
	const auto &table = (envval < 0) ? downTable : upTable;
	return static_cast<double>(table[amount]) / 65536.0;
}

static double ApplyITArpeggioFactor(const uint32 semitones, const bool periodsAreFrequencies)
{
	const double arpFactor = std::pow(2.0, semitones / 12.0);
	return periodsAreFrequencies ? arpFactor : 1.0 / arpFactor;
}

static double ApplyITArpeggioFactorReference(const uint32 semitones, const bool periodsAreFrequencies)
{
	const uint32 amount = std::min<uint32>(semitones * 16u, static_cast<uint32>(std::size(LinearSlideUpTable) - 1));
	const double arpFactor = static_cast<double>(LinearSlideUpTable[amount]) / 65536.0;
	return periodsAreFrequencies ? arpFactor : 1.0 / arpFactor;
}

static double ApplyITAutoVibratoFactor(const int vdelta, const bool periodsAreFrequencies)
{
	const double sign = periodsAreFrequencies ? 1.0 : -1.0;
	return std::pow(2.0, sign * vdelta / 768.0);
}

static double ApplyITAutoVibratoFactorReference(const int vdelta, const bool periodsAreFrequencies)
{
	const auto &upTable = periodsAreFrequencies ? LinearSlideUpTable : LinearSlideDownTable;
	const auto &downTable = periodsAreFrequencies ? LinearSlideDownTable : LinearSlideUpTable;
	const auto &fineUpTable = periodsAreFrequencies ? FineLinearSlideUpTable : FineLinearSlideDownTable;
	const auto &fineDownTable = periodsAreFrequencies ? FineLinearSlideDownTable : FineLinearSlideUpTable;
	const uint32 amount = static_cast<uint32>(std::abs(vdelta));
	const uint32 coarse = std::min<uint32>(amount / 4u, static_cast<uint32>(std::size(LinearSlideUpTable) - 1));
	const uint32 fine = std::min<uint32>(amount & 0x03u, static_cast<uint32>(std::size(FineLinearSlideUpTable) - 1));
	const auto &coarseTable = (vdelta < 0) ? downTable : upTable;
	const auto &fineTable = (vdelta < 0) ? fineDownTable : fineUpTable;
	uint32 factor = coarseTable[coarse];
	if(fine != 0)
	{
		factor += fineTable[fine] - 65536u;
	}
	return static_cast<double>(factor) / 65536.0;
}

static double ApplyInterpolatedAutoVibratoFactor(const int n, const bool periodsAreFrequencies)
{
	const double sign = periodsAreFrequencies ? 1.0 : -1.0;
	return std::pow(2.0, sign * n / (256.0 * 192.0));
}

static double ApplyInterpolatedAutoVibratoFactorReference(const int n, const bool periodsAreFrequencies)
{
	const auto &upTable = periodsAreFrequencies ? LinearSlideUpTable : LinearSlideDownTable;
	const auto &downTable = periodsAreFrequencies ? LinearSlideDownTable : LinearSlideUpTable;
	const int amount = std::abs(n);
	const uint32 index = std::min<uint32>(amount / 256, static_cast<uint32>(std::size(LinearSlideUpTable) - 2));
	const auto &table = (n < 0) ? downTable : upTable;
	const int df1 = table[index];
	const int df2 = table[index + 1];
	const int quarterStep = amount / 4;
	const int interpolated = df1 + ((df2 - df1) * (quarterStep & 0x3F) / 64);
	return static_cast<double>(interpolated) / 65536.0;
}

static double ApplyContinuousLinearPitchSlide(PitchT target, const int32 totalAmount, const bool periodsAreFrequencies)
{
	const double sign = periodsAreFrequencies ? 1.0 : -1.0;
	return target * std::pow(2.0, sign * totalAmount / 768.0);
}

static double ApplyITLinearPitchSlideReference(PitchT target, const int32 totalAmount, const bool periodsAreFrequencies)
{
	const auto &linearUpTable = periodsAreFrequencies ? LinearSlideUpTable : LinearSlideDownTable;
	const auto &linearDownTable = periodsAreFrequencies ? LinearSlideDownTable : LinearSlideUpTable;
	const auto &fineUpTable = periodsAreFrequencies ? FineLinearSlideUpTable : FineLinearSlideDownTable;
	const auto &fineDownTable = periodsAreFrequencies ? FineLinearSlideDownTable : FineLinearSlideUpTable;
	uint32 amount = std::abs(totalAmount);
	if(amount < 16u)
	{
		const auto &fineTable = (totalAmount > 0) ? fineUpTable : fineDownTable;
		return target * static_cast<double>(fineTable[amount]) / 65536.0;
	}
	const auto &linearTable = (totalAmount > 0) ? linearUpTable : linearDownTable;
	amount /= 4u;
	while(amount > 0)
	{
		const uint32 step = std::min<uint32>(amount, static_cast<uint32>(std::size(LinearSlideUpTable) - 1));
		target *= static_cast<double>(linearTable[step]) / 65536.0;
		amount -= step;
	}
	return target;
}

static double ApplyMicrotuningFactor(const int32 finetune)
{
	return std::pow(2.0, finetune / (12.0 * 256.0 * 128.0));
}

static double GetHertzFromNote(uint32 note, double nC5Speed)
{
	if(note == NOTE_NONE || note >= NOTE_MIN_SPECIAL)
	{
		return 0.0;
	}
	note -= NOTE_MIN;
	...
	double freq = nC5Speed * std::pow(2.0, (static_cast<int>(note) - 60) / 12.0);
	if(freq > static_cast<double>(int32_max))
	{
		freq = static_cast<double>(int32_max);
	}
	return freq;
}

static double GetReferenceHertzFromNote(uint32 note, double nC5Speed)
{
	if(note == NOTE_NONE || note >= NOTE_MIN_SPECIAL)
	{
		return 0.0;
	...
	const double tableFactor = static_cast<double>(LinearSlideUpTable[(note % 12u) * 16u]) / 65536.0;
	double freq = nC5Speed * tableFactor * std::ldexp(1.0, static_cast<int>(note / 12u) - 5);
	if(freq > static_cast<double>(int32_max))
	{
		freq = static_cast<double>(int32_max);
	}
...
static double GetXMLinearFreqFromPeriod(const uint32 period)
{
	if(period == 0)
	{
		return 0.0;
	}
	return 8363.0 * std::pow(2.0, (6.0 * 12.0 * 16.0 * 4.0 - period) / (12.0 * 16.0 * 4.0));
}

static double GetReferenceXMLinearFreqFromPeriod(const uint32 period)
{
	if(period == 0 || period > 29u * 768u)
	{
		return 0.0;
	}
	const uint32 octave = (period / 768u) + 2u;
...
static PitchT TranslateFTMPitch(uint16 pitch, ModChannel &chn, const CSoundFile &sndFile)
{
	PitchT period = sndFile.GetPeriodFromNote(NOTE_MIDDLEC - 12 + pitch / 16, chn.nFineTune, chn.nC5Speed);
...
static int8 MEDEnvelopeFromSample(const ModInstrument &instr, const CSoundFile &sndFile, uint8 envelope, uint16 envelopePos)
{
	...
	if(envelopePos >= mptSmp.nLength || !mptSmp.HasSampleData())
		return 0;

	return mptSmp.ReadSampleAsInt8(envelopePos);
}
...
void InstrumentSynth::States::ApplyChannelState(ModChannel &chn, PitchT &period, const CSoundFile &sndFile)
{
	if(!chn.pModInstrument || !chn.pModInstrument->synth.HasScripts())
		return;
...
void InstrumentSynth::States::State::ApplyChannelState(ModChannel &chn, PitchT &period, const CSoundFile &sndFile)
{
	if(m_volumeFactor != 16384)
		chn.nRealVolume = chn.nRealVolume * m_volumeFactor / 16384.0;
	if(m_volumeAdd != int16_min)
		chn.nRealVolume = std::clamp(chn.nRealVolume + static_cast<double>(m_volumeAdd), 0.0, 16384.0);
...
bool InstrumentSynth::States::State::EvaluateEvent(const Event &event, PlayState &playState, CHANNELINDEX channel, const CSoundFile &sndFile, States &states)
{
	...
	case Event::Type::FTM_AddPitch:
		if(event.i16)
		{
			...
			const PitchT limit = TranslateFTMPitch((event.i16 < 0) ? 0 : 0x21E, chn, sndFile);
...
void GlobalScriptState::ApplyChannelState(PlayState &playState, CHANNELINDEX chn, PitchT &period, const CSoundFile &sndFile)
{
	if(sndFile.m_globalScript.empty())
		return;
