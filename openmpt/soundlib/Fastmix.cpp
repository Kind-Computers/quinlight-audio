...
bool CSoundFile::MixChannel(int count, ModChannel &chn, CHANNELINDEX channel, bool doMix)
{
	if(chn.pCurrentSample || chn.nLOfs || chn.nROfs)
	{
		...
		if(chn.pModSample != nullptr)
		{
			...
			{
			case ModSample::RuntimeSampleFormat::Int16:
				functionNdx |= MixFuncTable::ndx16Bit;
				break;
			case ModSample::RuntimeSampleFormat::Float32:
				functionNdx |= MixFuncTable::ndxFloat;
				break;
			case ModSample::RuntimeSampleFormat::Float64:
				functionNdx |= MixFuncTable::ndxFloat | MixFuncTable::ndx16Bit;
				break;
			case ModSample::RuntimeSampleFormat::Int8:
			case ModSample::RuntimeSampleFormat::Auto:
			default:
				break;
			}
		}
		...
		do
		{
			...
			if(!doMix                                                   // Too many channels
			   || (!chn.nRampLength && chn.leftVol == 0 && chn.rightVol == 0))  // Channel is completely silent
			{
			...
			if (chn.nRampLength)
			{
				...
				{
					// Ramping is done
					chn.nRampPosition += chn.nRampLength;
					chn.nRampLength = 0;
